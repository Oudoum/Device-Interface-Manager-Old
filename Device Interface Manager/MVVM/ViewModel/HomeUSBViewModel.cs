using System.IO;
using System.Linq;
using System.Text.Json;
using System.Threading.Tasks;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using CommunityToolkit.Mvvm.Input;
using CommunityToolkit.Mvvm.ComponentModel;
using Device_Interface_Manager.interfaceIT.USB;
using static Device_Interface_Manager.MVVM.Model.HomeUSBModel;
using Device_Interface_Manager.MSFSProfiles;

namespace Device_Interface_Manager.MVVM.ViewModel;

public partial class HomeUSBViewModel : ObservableObject
{
    [ObservableProperty]
    private bool _isUSBEnabled = true;

    public ObservableCollection<string> BoardInfo { get; set; } = new();

    public ObservableCollection<string> BoardType { get; set; } = new();

    public ObservableCollection<Connection> Connections { get; set; } = new();

    public ObservableCollection<Profile> Profiles { get; set; } = new();

    public List<InterfaceIT_BoardInfo.Device> Devices { get; set; } = new();

    private List<USB> ListPMDG { get; set; } = new();

    public HomeUSBViewModel()
    {
        CreateProfiles();
        LoadUSBData();
        GetBoardInfo();

        if (Properties.Settings.Default.AutoHide && Connections.Count > 0)
        {
            StartUSB();
        }
    }

    public void GetBoardInfo()
    {
        BoardInfo.Clear();
        BoardType.Clear();
        BoardInfo.Add(string.Empty);
        BoardInfo.Add(string.Empty);
        BoardInfo.Add(string.Empty);
        BoardInfo.Add(string.Empty);
        BoardInfo.Add("                                          No USB device is found!");
    }

    public void GetBoardInfo(InterfaceIT_BoardInfo.BoardInformationStructure.BOARDCAPS bOARDCAPS)
    {
        BoardInfo.Clear();

        BoardInfo.Add("Board " + bOARDCAPS.szBoardType + " manufactured on " + bOARDCAPS.szManufactureDate + " has the following features: ");


        if (bOARDCAPS.dwFeatures == InterfaceIT_BoardInfo.Features.INTERFACEIT_FEATURE_NONE)
            BoardInfo.Add("No features programmed. Please obtain the update patch for this board.");
        else BoardInfo.Add(string.Empty);

        if ((bOARDCAPS.dwFeatures & InterfaceIT_BoardInfo.Features.INTERFACEIT_FEATURE_OUTPUT_LED) != 0)
            BoardInfo.Add(bOARDCAPS.nLEDCount + " | LEDs ( " + bOARDCAPS.nLEDFirst + " - " + bOARDCAPS.nLEDLast + " )");
        else BoardInfo.Add(string.Empty);


        if ((bOARDCAPS.dwFeatures & InterfaceIT_BoardInfo.Features.INTERFACEIT_FEATURE_INPUT_SWITCHES) != 0)
            BoardInfo.Add(bOARDCAPS.nSwitchCount + " | Switches ( " + bOARDCAPS.nSwitchFirst + " - " + bOARDCAPS.nSwitchLast + " )");
        else BoardInfo.Add(string.Empty);


        if ((bOARDCAPS.dwFeatures & InterfaceIT_BoardInfo.Features.INTERFACEIT_FEATURE_OUTPUT_7SEGMENT) != 0)
            BoardInfo.Add(bOARDCAPS.n7SegmentCount + " | 7 Segments ( " + bOARDCAPS.n7SegmentFirst + " - " + bOARDCAPS.n7SegmentLast + " )");
        else BoardInfo.Add(string.Empty);


        if ((bOARDCAPS.dwFeatures & InterfaceIT_BoardInfo.Features.INTERFACEIT_FEATURE_OUTPUT_DATALINE) != 0)
            BoardInfo.Add(bOARDCAPS.nDatalineCount + " | Datalines ( " + bOARDCAPS.nDatalineFirst + " - " + bOARDCAPS.nDatalineLast + " )");
        else BoardInfo.Add(string.Empty);


        //Not available
        if ((bOARDCAPS.dwFeatures & InterfaceIT_BoardInfo.Features.INTERFACEIT_FEATURE_OUTPUT_SERVO) != 0)
            BoardInfo.Add(bOARDCAPS.nServoController + " | Servos ( " + bOARDCAPS.nServoControllerFirst + " - " + bOARDCAPS.nServoControllerLast + " )");
        else BoardInfo.Add(string.Empty);


        if ((bOARDCAPS.dwFeatures & InterfaceIT_BoardInfo.Features.INTERFACEIT_FEATURE_SPECIAL_BRIGHTNESS) != 0)
            BoardInfo.Add("Brightness control supported");
        else BoardInfo.Add(string.Empty);


        if ((bOARDCAPS.dwFeatures & InterfaceIT_BoardInfo.Features.INTERFACEIT_FEATURE_SPECIAL_ANALOG_INPUT) != 0)
            BoardInfo.Add("Analog input supported (Single Channel)");
        else BoardInfo.Add(string.Empty);


        if ((bOARDCAPS.dwFeatures & InterfaceIT_BoardInfo.Features.INTERFACEIT_FEATURE_SPECIAL_ANALOG16_INPUT) != 0)
            BoardInfo.Add("Analog input supported (16 Channels)");
        else BoardInfo.Add(string.Empty);


        BoardType.Clear();

        foreach (var field in typeof(InterfaceIT_BoardIDs).GetFields())
        {
            if ((string)field.GetValue(null) == bOARDCAPS.szBoardType)
                BoardType.Add(field.Name.ToString().Replace('_', ' '));
        }
    }

    [RelayCommand]
    private async void StartUSB()
    {
        if (!(IsUSBEnabled = !IsUSBEnabled))
        {
            await StartUSBProfiles();
            return;
        }
            ListPMDG.ForEach(o => o.Stop());
            ListPMDG.Clear();
            SaveUSBData();
    }

    private async Task StartUSBProfiles()
    {
        foreach (var connection in Connections)
        {
            switch (connection.Profile.Id)
            {
                case 1:
                    MSFSProfiles.FENIX.A320.MCDU_L_USB mCDU_L_USB = new();
                    await Task.Run(() => mCDU_L_USB.Start(Devices.FirstOrDefault(o => o.SerialNumber == connection.Serial)));
                    ListPMDG.Add(mCDU_L_USB);
                    break;

                case 2:
                    MSFSProfiles.FENIX.A320.MCDU_R_USB mCDU_R_USB = new();
                    await Task.Run(() => mCDU_R_USB.Start(Devices.FirstOrDefault(o => o.SerialNumber == connection.Serial)));
                    ListPMDG.Add(mCDU_R_USB);
                    break;

                case 3:
                    MSFSProfiles.FBW.A32NX.MCDU_L_USB mCDU_L_USB1 = new();
                    await Task.Run(() => mCDU_L_USB1.Start(Devices.FirstOrDefault(o => o.SerialNumber == connection.Serial)));
                    ListPMDG.Add(mCDU_L_USB1);
                    break;

                case 4:
                    MSFSProfiles.FBW.A32NX.MCDU_R_USB mCDU_R_USB1 = new();
                    await Task.Run(() => mCDU_R_USB1.Start(Devices.FirstOrDefault(o => o.SerialNumber == connection.Serial)));
                    ListPMDG.Add(mCDU_R_USB1);
                    break;

                case 5:
                    MSFSProfiles.PMDG.B737.NG_CDU_L_USB nG_CDU_L_USB = new();
                    await Task.Run(() => nG_CDU_L_USB.Start(Devices.FirstOrDefault(o => o.SerialNumber == connection.Serial)));
                    ListPMDG.Add(nG_CDU_L_USB);
                    break;

                case 6:
                    MSFSProfiles.PMDG.B737.NG_CDU_R_USB nG_CDU_R_USB = new();
                    await Task.Run(() => nG_CDU_R_USB.Start(Devices.FirstOrDefault(o => o.SerialNumber == connection.Serial)));
                    ListPMDG.Add(nG_CDU_R_USB);
                    break;

                case 7:
                    MSFSProfiles.PMDG.B737.NG_MCP_USB nG_MCP_USB = new();
                    await Task.Run(() => nG_MCP_USB.Start(Devices.FirstOrDefault(o => o.SerialNumber == connection.Serial)));
                    ListPMDG.Add(nG_MCP_USB);
                    break;

                case 8:
                    MSFSProfiles.PMDG.B737.NG_MCP_777_USB nG_MCP_777_USB = new();
                    await Task.Run(() => nG_MCP_777_USB.Start(Devices.FirstOrDefault(o => o.SerialNumber == connection.Serial)));
                    ListPMDG.Add(nG_MCP_777_USB);
                    break;

                default:
                    break;
            }
        }
    }

    [RelayCommand]
    private void DeleteRow(Connection connection)
    {
        foreach (var conn in Connections)
        {
            if (conn == connection)
            {
                Connections.Remove(conn);
                return;
            }
        }
    }

    [RelayCommand]
    private void AddRow()
    {
        Connections.Add(new Connection() { Id = 0, Name = "NAME", Serial = "TEST", Profile = Profiles[0] });
    }

    private void CreateProfiles()
    {
        Profiles.Add(new Profile { Id = 0, Name = "-- None --" });

        Profiles.Add(new Profile { Id = 1, Name = "Fenix A320 Left MCDU" });
        Profiles.Add(new Profile { Id = 2, Name = "Fenix A320 Right MCDU" });

        Profiles.Add(new Profile { Id = 3, Name = "FBW A32NX Left MCDU" });
        Profiles.Add(new Profile { Id = 4, Name = "FBW A32NX Right MCDU" });

        Profiles.Add(new Profile { Id = 5, Name = "PMDG 737NG Left CDU" });
        Profiles.Add(new Profile { Id = 6, Name = "PMDG 737NG Right CDU" });

        Profiles.Add(new Profile { Id = 7, Name = "PMDG 737NG MCP" });
        Profiles.Add(new Profile { Id = 8, Name = "PMDG 737NG MCP (777)" });

        //this.Profiles.Add(new Profile { Id = , Name = "PMDG 737MAX Left CDU" });
        //this.Profiles.Add(new Profile { Id = , Name = "PMDG 737MAX Right CDU" });

        //this.Profiles.Add(new Profile { Id = , Name = "PMDG 777 Left CDU" });
        //this.Profiles.Add(new Profile { Id = , Name = "PMDG 777 Right CDU" });
        //this.Profiles.Add(new Profile { Id = , Name = "PMDG 777 Center CDU" });

        //this.Profiles.Add(new Profile { Id = , Name = "PMDG 747 Left CDU" });
        //this.Profiles.Add(new Profile { Id = , Name = "PMDG 747 Right CDU" });
        //this.Profiles.Add(new Profile { Id = , Name = "PMDG 747 Center CDU" });
    }

    [RelayCommand]
    private void ResetUSBScreens()
    {
        ListPMDG.ForEach(o => { o.pMDG737CDU.Top = 0; o.pMDG737CDU.Left = 0; o.fBWA32NXMCDU.Top = 0; o.fBWA32NXMCDU.Left = 0; });
    }

    private const string usb = @"Profiles\USB.json";
    private void LoadUSBData()
    {
        if (File.Exists(usb))
        {
            Connections = JsonSerializer.Deserialize<ObservableCollection<Connection>>(File.ReadAllText(usb));
        }
    }

    public void SaveUSBData()
    {
        if (Connections.Count == 0)
        {
            if (Directory.Exists((usb.Remove(8))))
            {
                File.Delete(usb);
            }
            return;
        }
        Directory.CreateDirectory(usb.Remove(8));
        string json = JsonSerializer.Serialize(Connections, new JsonSerializerOptions { WriteIndented = true });
        if (File.Exists(usb))
        {
            if (File.ReadAllText(usb) != json)
            {
                File.WriteAllText(usb, json);
            }
            return;
        }
        File.WriteAllText(usb, json);
    }
}