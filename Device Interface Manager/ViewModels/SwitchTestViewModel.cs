...
public partial class SwitchTestViewModel : ObservableObject, ISwitchLogChanged
{
    ...
    private InterfaceITAPI_Data.KeyNotifyCallback keyNotifiyCallback;
    ...
    private void KeyPressedProc(uint session, int key, int direction)
    {
        ...
        {
            SwitchLog.Add($"Session {session}, Switch {key} is now {(direction == (int)InterfaceITAPI_Data.SwitchDirection.Down ? "on" : "off")}");
        });
    }
}